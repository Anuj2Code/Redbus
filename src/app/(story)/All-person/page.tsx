...
export default async function AllPerson({ id }: props) {
    ...
    // Combine UserOne and UserTwo, then exclude the current user
    ...
    return (
        <div className="h-[90vh] overflow-y-scroll bg-black bg-grid-white/[0.2]">
            ...
            <h1 className="w-full text-center text-muted-foreground top-24 relative text-4xl">
                Connected People
            </h1>
            <div className="h-full w-full flex justify-center items-center flex-col">
                {filteredUsers &&
                    filteredUsers.map((item, idx: any) => {
                        return (
                            <div className="flex-col flex p-3 cursor-pointer" key={idx}>
                                <Link href={`/chat-author/${item.id}`}>
                                    <div className="flex gap-x-3 min-w-[300px] items-center justify-around rounded-xl hover:bg-orange-600 duration-200 h-[60px] transition-all">
                                        {/* @ts-ignore: UserAvatar is correctly used but TypeScript may not recognize the props */}
                                        <UserAvatar
                                            src={item.imageUrl && item.imageUrl || "GS"}
                                            username={item.userName!} // Use non-null assertion (!) for optional properties
                                        />
