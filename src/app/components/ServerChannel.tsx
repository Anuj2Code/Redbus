...
export default function ServerChannel({ channel, server, role }: serverChannelProps) {
    ...
    return (
        ...
        >
            ...
            <p onClick={onNavigate} className={cn("line-clamp-1 font-semibold text-sm text-zinc-500 group-hover:text-zinc-400 transition", params?.channelId === channel.id && "text-primary")}>{channel.name}</p>
