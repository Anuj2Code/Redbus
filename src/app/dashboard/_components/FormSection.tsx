...
export default function FormSection(params: props) {
    ...
    return (
        <Card className="w-[350px] border-t-0 border-l-0 rounded-r-none border-b-0 bg-black border-r-4 border-zinc-500">
            <CardHeader>
                ...
                <Form {...form}>
                    <form onSubmit={form.handleSubmit(onSubmit)}>
                        <FormField
                            ...
                            render={({ field }) => (
                                <FormItem>
                                    <FormLabel className="text-muted-foreground font-bold ">{templateData?.FormHeader}
                                    </FormLabel>
                                    <FormControl>
                                        <Input id="name"
                                            disabled={params.loading}
                                            className="focus-visible:ring-0 focus-visible:ring-offset-0 " placeholder="Type here to start"
                                            {...field}
                                        />
                                    </FormControl>
                        ...
                        <div className="flex justify-around mt-4">
                            {params.loading ? (
                                <Button disabled className="bg-blue-500 absolute justify-end text-white hover:bg-sky-700 transition-all duration-150">
                                    <Loader2 className="mr-2 w-4 h-4 animate-spin" />
                                </Button>
                            ) : (
                                <Button type="submit" disabled={!checkPre && creditPoint>=5} className="bg-blue-500 absolute justify-end text-white hover:bg-sky-700 transition-all duration-150">Generate</Button>
                            )}
                        </div>
                    </form>
                </Form>
            <Button disabled={load} onClick={()=>saveDataDb(params.response,params.prm)}  className="bg-orange-500 text-white hover:bg-orange-700 transition-all duration-150">Save</Button>
            </CardContent>
        </Card>
    )
}
